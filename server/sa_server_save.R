...
output$constant_lagk <- renderPlot({
  ...
  #----------------------------------------------------------------------------    
  upstr.df <- upstr.df %>% 
    dplyr::left_join(pot_withdrawals.sub, by = "date") %>% 
  #----------------------------------------------------------------------------    
    select(date, lfalls, por, lfalls_from_upstr, lfalls_trigger) %>% 
...
#  sub.df <- dplyr::filter(upstr.df, gage %in% input$gages.sa)
  sub.df <- dplyr::filter(final.df, gage %in% input$gages.sa)
  #--------------------------------------------------------------------------
    validate(
      need(nrow(sub.df) != 0,
...
#  final.plot <- ggplot(sub.df, aes(x = date, y = flow,
  final.plot <- ggplot(final.df, aes(x = date, y = flow,
